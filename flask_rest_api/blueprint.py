...
class Blueprint(
    ...
    def _store_endpoint_docs(self, endpoint, obj, parameters, **options):
        ...
        endpoint_auto_doc = self._auto_docs.setdefault(endpoint, OrderedDict())
        endpoint_manual_doc = self._manual_docs.setdefault(endpoint, OrderedDict())
        ...
        if openapi_version.major < 3:
            if 'responses' in operation:
                for resp in operation['responses'].values():
                    if 'example' in resp:
                        resp['examples'] = {'application/json': resp.pop('example')}
        else:
            if 'responses' in operation:
                for resp in operation['responses'].values():
