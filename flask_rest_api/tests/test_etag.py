...
from flask_rest_api.exceptions import CheckEtagNotCalledError
from flask_rest_api.exceptions import NotModified
from flask_rest_api.exceptions import PreconditionFailed
from flask_rest_api.exceptions import PreconditionRequired
...
class TestEtag:
    def test_etag_is_deterministic(self):
        ...
        data_copies = [
            OrderedDict([('b', 2), ('a', 1), ('c', OrderedDict([('a', 1), ('b', 2)]))]),
            OrderedDict([('a', 1), ('b', 2), ('c', OrderedDict([('b', 2), ('a', 1)]))]),
            OrderedDict([('a', 1), ('c', OrderedDict([('a', 1), ('b', 2)])), ('b', 2)]),
            OrderedDict([('c', OrderedDict([('a', 1), ('b', 2)])), ('b', 2), ('a', 1)]),
        ]
    ...
    def test_etag_check_etag(self, app, schemas, etag_disabled):
        ...
        with app.test_request_context('/', headers={'If-Match': old_etag}):
            ...
            if not etag_disabled:
                ...
                blp.check_etag(new_item)
        with app.test_request_context('/', headers={'If-Match': old_etag_with_schema}):
    ...
    def test_etag_verify_check_etag_warning(self, app, method):
        ...
        with mock.patch.object(app.logger, 'warning') as mock_warning:
            with app.test_request_context(
                '/', method=method, headers={'If-Match': old_etag}
            ):
    ...
    def test_etag_verify_check_etag_exception(self, app, method, debug, testing):
        ...
        with app.test_request_context('/', method=method):
            if (debug or testing) and method in ['PUT', 'PATCH', 'DELETE']:
                with pytest.raises(
                    CheckEtagNotCalledError, match='ETag not checked in endpoint'
                ):
                    blp._verify_check_etag()
            else:
                blp._verify_check_etag()
    ...
    def test_etag_set_etag(self, app, schemas, etag_disabled):
        ...
        with app.test_request_context('/', headers={'If-None-Match': etag}):
            if not etag_disabled:
                ...
                blp.set_etag(item)
                assert 'etag' not in _get_etag_ctx()
        with app.test_request_context('/', headers={'If-None-Match': etag_with_schema}):
            if not etag_disabled:
                with pytest.raises(NotModified):
    ...
    @pytest.mark.parametrize('etag_disabled', (True, False))
    @pytest.mark.parametrize('method', HTTP_METHODS)
    def test_set_etag_method_not_allowed_warning(self, app, method, etag_disabled):
        app.config['ETAG_DISABLED'] = etag_disabled
        blp = Blueprint('test', __name__)
    ...
    def test_etag_operations_etag_disabled(self, app_with_etag):
        ...
        response = client.get(
            '/test/{}'.format(item_1_id), headers={'If-None-Match': 'dummy_etag'}
        )
        assert response.status_code == 200
        ...
        response = client.delete(
            '/test/{}'.format(item_2_id), headers={'If-Match': 'dummy_etag'}
        )
        assert response.status_code == 204
