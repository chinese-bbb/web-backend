...
class TestApi:
    ...
    @pytest.mark.parametrize('openapi_version', ['2.0', '3.0.2'])
    def test_api_register_converter_before_and_after_init(self, app, openapi_version):
        api = Api()
        blp = Blueprint('test', 'test', url_prefix='/test')
    ...
    @pytest.mark.parametrize(
        'mapping',
        [('custom string', 'custom'), ('custom string', None), (ma.fields.Integer,)],
    )
    ...
    def test_api_openapi_version_parameters(self, app):
        ...
        with pytest.raises(
            OpenAPIVersionNotSpecified, match='The OpenAPI version must be specified'
        ):
            Api(app)
